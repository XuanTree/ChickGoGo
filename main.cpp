...
static fs::path getExeDirectory() {
    ...
    if (GetModuleFileNameW(nullptr, path, MAX_PATH) == 0) {
        std::cerr << "[main] Failed to get exe path" << std::endl;
...
// ============================================================
static bool findAndSetAssetsDirectory() {
    ...
    while (true) {
        ...
        if (parent == searchDir) {
            // 已到根目录，未找到
            std::cerr << "[main] Could not find assets/ directory from: "
